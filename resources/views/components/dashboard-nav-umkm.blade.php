            ...
            <div class="flex justify-between items-center h-20">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-3 group shrink-0">
                    <div class="relative w-10 h-10 flex items-center justify-center">
                        <div
                            class="absolute inset-0 bg-[#2563EB] rounded-xl rotate-3 group-hover:rotate-6 transition-all shadow-lg shadow-[#2563EB]/20">
                        </div>
                        <span class="relative text-white font-display font-bold text-lg">K</span>
                    </div>
                    <span class="font-display font-bold text-xl tracking-tight">Konekin<span
                            class="text-[#2563EB]">.</span></span>
                </a>

                <!-- Navigation Links -->
                <div
                    class="hidden md:flex items-center bg-white/50 px-6 py-2 rounded-full border border-white/80 shadow-sm gap-1">
                    <a href="{{ route('dashboard.umkm') }}"
                        class="px-4 py-2 {{ request()->routeIs('dashboard.umkm') ? 'bg-[#2563EB] text-white shadow-md shadow-[#2563EB]/20' : 'text-[#1E3A8A]/70 hover:text-[#2563EB]' }} rounded-full text-sm font-bold transition-all">Dashboard</a>
                    <a href="{{ route('kreator.index') }}"
                        class="px-4 py-2 {{ request()->routeIs('kreator.index') ? 'bg-[#2563EB] text-white shadow-md shadow-[#2563EB]/20' : 'text-[#1E3A8A]/70 hover:text-[#2563EB]' }} rounded-full text-sm font-bold transition-all">Cari
                        Kreator</a>
                    <a href="{{ route('rekomendasi.kreator') }}"
                        class="px-4 py-2 {{ request()->routeIs('rekomendasi.kreator') ? 'bg-[#2563EB] text-white shadow-md shadow-[#2563EB]/20' : 'text-[#1E3A8A]/70 hover:text-[#2563EB]' }} rounded-full text-sm font-bold transition-all">Rekomendasi
                        AI</a>
                    <a href="{{ route('projects.create') }}"
                        class="px-4 py-2 {{ request()->routeIs('projects.create') ? 'bg-[#2563EB] text-white shadow-md shadow-[#2563EB]/20' : 'text-[#1E3A8A]/70 hover:text-[#2563EB]' }} rounded-full text-sm font-bold transition-all">Upload
                        Proyek</a>
                    <a href="{{ route('projects.progress') }}"
                        class="px-4 py-2 {{ request()->routeIs('projects.progress') || request()->routeIs('projects.progress.update') ? 'bg-[#2563EB] text-white shadow-md shadow-[#2563EB]/20' : 'text-[#1E3A8A]/70 hover:text-[#2563EB]' }} rounded-full text-sm font-bold transition-all">Progress
                        Proyek</a>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    @php
                        $overdueProjects = \App\Models\Project::where('client_id', auth()->id())
                            ...
                            ->get()
                            ->map(function ($p) {
                                $deadline = \Illuminate\Support\Carbon::parse($p->deadline);
                                $appsCount = \App\Models\ProjectApplication::where('project_id', $p->id)->count();

                                if ($appsCount === 0) {
                                    $p->reason = 'Tidak ada yang apply sampai batas waktu.';
                                } elseif (empty($p->selected_creative_id)) {
                                    $p->reason = 'Batas waktu habis tapi kamu belum pilih pelamar.';
                                } else {
                    ...
                    <div class="relative" id="notif-wrapper">
                        <button onclick="toggleNotifDropdown(event)"
                            class="relative p-2.5 bg-white border border-[#2563EB]/10 rounded-xl hover:bg-[#EFF6FF] transition-all group">
                            <svg class="w-6 h-6 text-[#1E3A8A]/70 group-hover:text-[#2563EB]" fill="none"
                            ...
                            @if($totalNotificationCount > 0)
                                <span class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center">
                                    ...
                                    <span
                                        class="relative inline-flex items-center justify-center rounded-full h-5 w-5 bg-red-500 text-[10px] font-bold text-white border-2 border-white">
                        ...
                        <div id="notif-dropdown"
                            class="absolute hidden right-0 mt-4 w-80 bg-white rounded-[2rem] shadow-2xl shadow-[#1E3A8A]/20 border border-[#2563EB]/10 overflow-hidden z-[60]">
                            ...
                            <div class="p-5 border-b border-slate-50 flex items-center justify-between">
                                ...
                                <span
                                    class="px-3 py-1 rounded-full bg-[#EFF6FF] text-[#2563EB] text-[10px] font-black uppercase tracking-widest">{{ $totalNotificationCount }}
                                    Baru</span>
                            </div>
                            ...
                            <div class="max-h-96 overflow-y-auto">
                                ...
                                @empty
                                    <div class="p-10 text-center">
                                        ...
                                        <p class="text-sm font-bold text-[#1E3A8A]/40 uppercase tracking-[0.2em]">Tidak ada
                                            notifikasi</p>
                                    </div>
                                @endforelse
                            </div>
                    ...
                    <script>
                        ...
                        // Tutup dropdown kalau klik di mana saja
                    ...
                    <a href="{{ route('profile.index') }}"
                        class="flex items-center gap-3 pl-4 border-l border-[#2563EB]/10 group/profile cursor-pointer hover:opacity-80 transition-all">
                        <div class="text-right hidden sm:block">
                            <p
                                class="text-sm font-bold text-[#1E3A8A] leading-none group-hover/profile:text-[#2563EB] transition-colors">
                                {{ auth()->user()->name }}</p>
                            <p class="text-[10px] font-bold text-[#2563EB] uppercase tracking-wider mt-1">UMKM</p>
                        </div>
                        <div
                            class="w-11 h-11 rounded-xl bg-gradient-to-br from-[#2563EB] to-[#0A66C2] p-0.5 shadow-lg shadow-[#2563EB]/20 group-hover/profile:scale-105 transition-transform">
                            <img src="{{ auth()->user()->profile_photo ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=random' }}"
                                alt="Profile" class="w-full h-full object-cover rounded-[10px] border-2 border-white">
                        </div>
                    </a>

                    <!-- Logout Button -->
                    <button onclick="confirmLogout()"
                        class="p-2.5 bg-white border border-red-100 rounded-xl hover:bg-red-50 transition-all group ml-1"
                        title="Logout">
                        <svg class="w-6 h-6 text-red-500 group-hover:text-red-600 transition-colors" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="logout-modal" class="fixed inset-0 z-[100] hidden">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <!-- Modal panel -->
                <div
                    class="relative transform overflow-hidden rounded-[3rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-white/20">
                    <div class="bg-white px-6 pt-10 pb-8 sm:px-10">
                        <div class="flex flex-col items-center">
                            <!-- User Profile Info -->
                            <div class="relative mb-6">
                                <div
                                    class="w-24 h-24 rounded-3xl bg-gradient-to-br from-[#2563EB] to-[#0A66C2] p-1 shadow-2xl shadow-[#2563EB]/20 rotate-3 transition-transform hover:rotate-0">
                                    <img src="{{ auth()->user()->profile_photo ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=random' }}"
                                        alt="Profile"
                                        class="w-full h-full object-cover rounded-[20px] border-4 border-white">
                                </div>
                                <div
                                    class="absolute -bottom-2 -right-2 bg-red-500 text-white p-2 rounded-xl shadow-lg border-4 border-white">
