    ...
    <main class="pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        <div class="mb-12">
            <a href="{{ route('dashboard.umkm') }}"
                class="inline-flex items-center gap-2 text-[#2563EB] font-bold text-sm hover:gap-3 transition-all mb-6">
            ...
            <p class="text-[#1E3A8A]/60 font-medium text-lg">Bagikan ide atau kebutuhan bisnismu dan biarkan talenta
                terbaik menemukannya.</p>
        </div>
        ...
        <div class="bg-white rounded-[3rem] p-8 md:p-12 border border-[#2563EB]/5 shadow-2xl shadow-[#2563EB]/5">
            <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        <label for="category"
                            class="text-sm font-extrabold text-[#1E3A8A] uppercase tracking-wider ml-1">Kategori</label>
                        <select id="category" name="category" required
                            ...
                            <option value="Branding" {{ old('category') === 'Branding' ? 'selected' : '' }}>Branding
                            </option>
                            <option value="Social Media" {{ old('category') === 'Social Media' ? 'selected' : '' }}>Social
                                Media</option>
                            <option value="Web Dev" {{ old('category') === 'Web Dev' ? 'selected' : '' }}>Web Development
                            </option>
                            <option value="Videography" {{ old('category') === 'Videography' ? 'selected' : '' }}>
                                Videography</option>
                            <option value="UI/UX Design" {{ old('category') === 'UI/UX Design' ? 'selected' : '' }}>UI/UX
                                Design</option>
                            <option value="Illustration" {{ old('category') === 'Illustration' ? 'selected' : '' }}>
                                Illustration</option>
                        </select>
                ...
                <div class="space-y-3">
                    <label for="deadline"
                        class="text-sm font-extrabold text-[#1E3A8A] uppercase tracking-wider ml-1">Deadline
                        Pengerjaan</label>
                    <input type="date" id="deadline" name="deadline" required value="{{ old('deadline') }}"
                        min="{{ now()->toDateString() }}"
                        class="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-[#2563EB] outline-none transition-all font-medium">
                    <p class="text-sm text-[#1E3A8A]/60 font-medium">Tanggal ini akan terlihat oleh creative worker
                        sebagai batas pengumpulan/pengerjaan proyek.</p>
                ...
                <div class="space-y-3">
                    <label for="requirements"
                        class="text-sm font-extrabold text-[#1E3A8A] uppercase tracking-wider ml-1">Kebutuhan /
                        Spesifikasi (Opsional)</label>
                    <textarea id="requirements" name="requirements" rows="3"
                        placeholder="Contoh: Harus berpengalaman di industri F&B, paham gaya minimalis, dll."
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <label for="project_media"
                                class="text-sm font-extrabold text-[#1E3A8A] uppercase tracking-wider ml-1">Upload Foto
                                / Video (Opsional)</label>
                            <p class="text-sm text-[#1E3A8A]/60 font-medium mt-2">Tambahkan referensi visual supaya
                                creative worker lebih cepat memahami brief. JPG, PNG, MP4, MOV, WEBM maksimal 20MB.</p>
                        </div>
                    </div>
                    ...
                    <div class="rounded-[2.5rem] border border-dashed border-[#2563EB]/20 bg-[#F8FAFC] p-6">
                        <div class="grid grid-cols-1 lg:grid-cols-[1fr_0.8fr] gap-6 items-center">
                            <label for="project_media"
                                class="flex items-center justify-center min-h-[220px] rounded-[2rem] bg-white border border-[#2563EB]/10 cursor-pointer hover:border-[#2563EB]/25 transition-all overflow-hidden">
                                <div id="media-placeholder" class="text-center px-6">
                                    <div
                                        class="w-14 h-14 rounded-2xl bg-[#EFF6FF] text-[#2563EB] flex items-center justify-center mx-auto mb-4">
                                    ...
                                    <p class="text-sm text-[#1E3A8A]/55 font-medium mt-2">Bisa berupa foto produk, brief
                                        visual, atau video referensi.</p>
                                </div>
                                <img id="media-image-preview" class="hidden w-full h-full object-cover"
                                    alt="Preview media">
                                <video id="media-video-preview" class="hidden w-full h-full object-cover"
                                    controls></video>
                            </label>
                            ...
                            <div class="space-y-4">
                                ...
                                <button type="button" onclick="document.getElementById('project_media').click()"
                                    class="w-full px-6 py-4 rounded-2xl bg-[#1E3A8A] text-white font-bold text-sm hover:bg-[#2563EB] transition-all">
                                    Pilih File Referensi
                                </button>
                                <div class="rounded-[1.8rem] bg-white border border-[#2563EB]/10 p-5">
                                    <p
                                        class="text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#2563EB] mb-2">
                                        Kenapa ini penting?</p>
                                    <p class="text-sm text-[#1E3A8A]/65 leading-7 font-medium">Media referensi membantu
                                        creative worker melihat vibe, kebutuhan, dan konteks proyek sejak awal. Ini
                                        opsional, tapi sangat membantu menaikkan kualitas apply yang masuk.</p>
                                </div>
                ...
                <div class="p-6 bg-blue-50 rounded-3xl border border-blue-100/50 flex gap-4">
                    <div
                        class="shrink-0 w-10 h-10 bg-white rounded-xl flex items-center justify-center text-[#2563EB] shadow-sm">
                    ...
                    <p class="text-sm text-[#1E3A8A]/80 font-medium leading-relaxed">
                        <span class="font-bold text-[#2563EB]">Tips:</span> Berikan deskripsi yang jelas dan anggaran
                        yang realistis untuk mendapatkan tawaran terbaik dari para profesional kreatif kami.
                    </p>
                </div>

                <div
                    class="rounded-3xl border border-[#2563EB]/10 bg-gradient-to-r from-white to-[#F8FAFC] p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                    <div>
                        <p class="text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#2563EB] mb-2">
                            Rekomendasi Kreator AI</p>
                        <p class="text-sm text-[#1E3A8A]/70 font-medium leading-7 max-w-2xl">
                            Belum yakin siapa creative worker yang paling cocok? Buka halaman rekomendasi AI untuk
                            mencocokkan data UMKM dengan kreator terbaik dari model machine learning.
                        </p>
                    </div>
                    <a href="{{ route('rekomendasi.kreator') }}"
                        class="inline-flex items-center justify-center px-6 py-4 rounded-2xl bg-[#2563EB] text-white font-bold text-sm hover:bg-[#1E3A8A] transition-all shadow-lg shadow-[#2563EB]/20 shrink-0">
                        Buka Rekomendasi
                    </a>
                </div>
                ...
                <!-- Submit Button -->
                <button type="submit"
                    class="w-full py-5 bg-[#1E3A8A] text-white rounded-2xl font-bold text-lg hover:bg-[#2563EB] transition-all shadow-xl shadow-[#1E3A8A]/10 active:scale-[0.98]">
